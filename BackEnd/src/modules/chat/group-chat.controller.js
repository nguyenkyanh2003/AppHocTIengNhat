...
export const upload = multer({
    ...
    fileFilter: (req, file, cb) => {
        const allowedTypes = /jpeg|jpg|png|gif|pdf|doc|docx|txt|wav|mp3|ogg|webm|m4a/;
        ...
        if (mimetype && extname) {
            ...
            cb(new Error('File không hợp lệ. Chỉ chấp nhận: image, pdf, doc, txt, audio'));
        }
    }
});
...
export const postByGroupID = async (req, res) => {
    try {
        const { groupID } = req.params;
        const userId = req.user._id;
        ...
        const populatedMessage = await GroupChat.findById(newMessage._id)
            .populate('user_id', 'HoTen TenDangNhap AnhDaiDien')
            .populate({
                path: 'reply_to',
                select: 'content user_id created_at',
                populate: {
                    path: 'user_id',
                    select: 'HoTen TenDangNhap'
                }
            })
            .lean();
...
export const postByGroupIDUpload = async (req, res) => {
    try {
        const { groupID } = req.params;
        const userId = req.user._id;
        ...
        const newMessage = await GroupChat.create({
            group_id: groupID,
            user_id: userId,
            content: content || (fileType === 'AUDIO' ? '🎤 Tin nhắn thoại' : 'Đã gửi file đính kèm'),
            type: fileType,
...
export const getByGroupIDLatest = async (req, res) => {
    try {
        const { groupID } = req.params;
...
export const putByGroupIDByMessageID = async (req, res) => {
    try {
...
// API Xóa tin nhắn 
export const deleteByGroupIDByMessageID = async (req, res) => {
    try {
        const { groupID, messageID } = req.params;
        const userId = req.user._id;
...
export const getByGroupIDSearch = async (req, res) => {
    try {
        const { groupID } = req.params;
...
export const getByGroupIDStatistics = async (req, res) => {
    try {
        const { groupID } = req.params;
...
export const deleteAdminByGroupIDClear = async (req, res) => {
    try {
        const { groupID } = req.params;
