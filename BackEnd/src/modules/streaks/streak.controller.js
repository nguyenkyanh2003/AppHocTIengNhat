...
export const getMyStreak = async (req, res) => {
  try {
    ...
    if (!streak) {
      ...
      // Cập nhật streak cho hoạt động đầu tiên (đăng nhập)
      streak.updateStreakOnActivity();
      await streak.save();
    } else {
      // Kiểm tra và cập nhật streak khi user load dữ liệu (đăng nhập hoặc refresh)
      const streakResult = streak.checkAndUpdateStreak();
      if (streakResult.need_save) {
        await streak.save();
        console.log(`🔄 Streak updated on login - User: ${username}, Streak: ${streak.current_streak} days, Broken: ${streakResult.streak_broken || false}`);
      }
      console.log(`✅ Streak found - User: ${username}, XP: ${streak.total_xp}, Streak: ${streak.current_streak} days, Level: ${streak.level}, Last activity: ${streak.last_activity_date}`);
    }
...
export const postAddXp = async (req, res) => {
  try {
...
export const getLeaderboard = async (req, res) => {
  try {
    ...
    const leaderboard = await UserStreak.find(query)
      ...
      .populate('user', 'TenDangNhap Email HoTen AnhDaiDien');
...
export const postTestResetYesterday = async (req, res) => {
  try {
    const streak = await UserStreak.findOne({ user: req.user._id });
...
export const getTestDebug = async (req, res) => {
  try {
    const streak = await UserStreak.findOne({ user: req.user._id });
