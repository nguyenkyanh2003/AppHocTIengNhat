...
class _GroupDetailScreenState extends State<GroupDetailScreen>
    ...
    return Column(
      children: [
        ...
        AudioRecorderWidget(
          ...
          onAudioRecorded: (path) => _showAudioOptions(path, chatProvider),
        ),
          ...
          child: Row(
            children: [
              IconButton(
                ...
                onPressed: () => _pickAndSendFile(chatProvider),
              ...
              IconButton(
                ...
                onPressed: () => _startAudioRecording(chatProvider),
  ...
  Widget _buildMessageBubble(
      GroupMessage message, bool isMe, GroupChatProvider chatProvider) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Flexible(
            child: GestureDetector(
              onLongPress: () =>
                  _showMessageOptions(message, isMe, chatProvider),
              child: Column(
                crossAxisAlignment:
  ...
  void _deleteMessage(
      GroupMessage message, GroupChatProvider chatProvider) async {
    final confirm = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
  ...
  Future<void> _pickAndSendImage(GroupChatProvider chatProvider) async {
    try {
      final ImagePicker picker = ImagePicker();
      final XFile? image = await picker.pickImage(
        source: ImageSource.gallery,
      ...
      if (image != null) {
        ...
        await _sendFileMessage(chatProvider, bytes, fileName);
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _pickAndSendFile(GroupChatProvider chatProvider) async {
    try {
      ...
      if (result != null && result.files.isNotEmpty) {
        ...
        if (file.bytes != null) {
          await _sendFileMessage(chatProvider, file.bytes!, file.name);
        } else {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _startAudioRecording(GroupChatProvider chatProvider) {
    _recorderController.start();
  }

  Future<void> _showAudioOptions(
      String audioPath, GroupChatProvider chatProvider) async {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              Navigator.pop(context);
              await _sendAudioFile(chatProvider, audioPath);
            },
