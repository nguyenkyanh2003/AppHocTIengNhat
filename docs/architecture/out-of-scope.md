...
| Study groups/group chat | Đóng băng code, route và UI hiện có; không thêm matching, Pomodoro/presence, WebSocket hoặc refactor module trong Phase 2. Demo chỉ mở danh sách/chi tiết nhóm đã seed ở mức xem, không chat hoặc upload media. | Vòng lặp SRS/progress đã ổn định; feature xã hội có trạng thái hữu ích với 1–2 tài khoản seed; có ngân sách refactor/test/moderation. Hướng mở lại ưu tiên là mục tiêu tuần + endpoint tổng hợp tiến độ, không phải matching. Khi mở lại chat phải cập nhật roadmap, acceptance test và security plan cho message type, upload, moderation và rate limit. |
